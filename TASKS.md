...
- Layout approach: simple text UI with highlighted row; reserve top status line for playback state/icon + volume, body for list items with scroll window, footer hint for buttons (e.g., Back/Play-Pause). Use ellipsis/truncation for long labels; keep 6–8 visible rows.
